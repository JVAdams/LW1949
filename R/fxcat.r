...
#' @param dat
#'   A data frame of raw toxicity data, including these three variables:
#'	   dose (the concentration of the applied chemical),
#'     ntot (the number of individuals tested), and
#'     nfx (the number of affected individuals).
#' @return
#'   A numeric vector the same length as \code{prob} with
#'     quantiles on the probit scale.
#' @export
#' @examples
#' test <- data.frame(
