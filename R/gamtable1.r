...
#' @return		A \code{\link{gamObject}} that can be used to predict the corrected values (as proportions) from \code{distexpprop5}, 
#'	the distance from the expected values (as proportions) and 0.5
#' @import 		mgcv
#' @export
#' @details		Note that for an expected value of 37 Table 1 gives a corrected value of 9.4, but for an expected value of 63
...
gamtable1 <- function() {
	...
	# note that for t1expected=37 Table 1 shows 9.4 and for t1expected=63 it shows 90.5, so I split the difference and used 9.45
