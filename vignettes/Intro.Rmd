...
- log~10~ transformed dose (`log10dose`), 
...
- `params`, the estimated intercept and slope on the log~10~-probit scale; and
...
Use the `plotDE` function to plot the raw data on the log~10~-probit scale.  And use the `abline` function with the two fitted models to add their predicted relations to the plot.  Observations with no or 100% affected are plotted at 0.1 and 99.9% respectively using white filled circles.
