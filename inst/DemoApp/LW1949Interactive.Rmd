...
```{r, echo=FALSE}
# plot of dose vs. % affected
output$onePlot <- renderPlot(
  {
    y <- probit(predLW()[, "pct"]/100)
    xr <- range(mydat()$dose[!is.na(mydat()$dose) & mydat()$dose>0], 
      range(predLW()[, c("ED", "lower", "upper")], na.rm=TRUE), na.rm=TRUE)
    par(mar=c(4, 4, 1, 0.1))
    plotDE(mydat(), xlim=xr)
    abline(fLW()$params)
...
Litchfield, J. T. Jr. and F. Wilcoxon. 1949. [A simplified method of evaluating dose-effect experiments](http://jpet.aspetjournals.org/content/96/2/99.abstract). *Journal of Pharmacology and Experimental Therapeutics* 99(2):99-113.
